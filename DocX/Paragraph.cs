...
namespace Novacode
{
    ...
    public class Paragraph
    {
        ...
        public void SetTabStops(IList<int> tabStops)
        {
            SetTabStops(tabStops.Select(pos => new TabStop {Position = pos, Type = TabStopType.Left}).ToList());
        }

        public void SetTabStops(IList<TabStop> tabStops)
        {
            XElement pPr = xml.Element(XName.Get("pPr", DocX.w.NamespaceName));
            if (pPr == null)
            ...
            XElement tabs = pPr.Element(XName.Get("tabs", DocX.w.NamespaceName));
            if (tabs != null)
                tabs.Remove();

            pPr.Add(new XElement(XName.Get("tabs", DocX.w.NamespaceName)));
            tabs = pPr.Element(XName.Get("tabs", DocX.w.NamespaceName));

            if (tabStops != null)
            {
                foreach (var tabStop in tabStops)
                {
                    tabs.Add(new XElement(XName.Get("tab", DocX.w.NamespaceName),
                                          new XAttribute(XName.Get("val", DocX.w.NamespaceName), tabStop.Type.GetValue()),
                                          new XAttribute(XName.Get("pos", DocX.w.NamespaceName), tabStop.Position.ToString())));
                }
            }
        }
