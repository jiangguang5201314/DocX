...
namespace Novacode
{
    ...
    public class DocX: IDisposable
    {
        ...
        public void RemoveParagraph(Paragraph p)
        {
            ...
            var paragraphPair = this.paragraphLookup.Single(pair => pair.Value == p);
            this.paragraphLookup.Remove(paragraphPair.Key);

            IEnumerable<XElement> paras = this.mainDoc.Descendants(XName.Get("p", "http://schemas.openxmlformats.org/wordprocessingml/2006/main"));
            foreach (XElement par in paras)
            {
                if (par.Value == p.Text)
                {
        ...
        public void AddPageBreak()
        {
            ...
            Paragraphs.Last().xml.AddAfterSelf(p);
        }
        ...
        /// <summary>
        /// Replaces text with several lines of text that can have formatting.  The text that you're
        /// replacing must be on a line by itself.
        /// </summary>
        /// <param name="oldValue"></param>
        /// <param name="lines"></param>
        /// <param name="trackChanges"></param>
        public void ReplaceTextWithLines(string oldValue, IList<LineWithFormatting> lines, bool trackChanges)
        {
            foreach (Paragraph p in ((IEnumerable<Paragraph>) paragraphs).Reverse())
            {
                if (p.Text == oldValue)
                {
                    var paragraphBefore = GetParagraphBefore(p);
                    var paragraphBeforeIndex = paragraphBefore != null
                                                   ? paragraphs.IndexOf(paragraphBefore)
                                                   : (int?) null;
                    RemoveParagraph(p);

                    var firstNewParagraphIndex = paragraphBeforeIndex.GetValueOrDefault() + 1;
                    foreach (var line in lines)
                    {
                        var insertIndex = paragraphBeforeIndex != null
                                              ? paragraphs[paragraphBeforeIndex.Value].endIndex
                                              : 0;

                        Paragraph newParagraph = new Paragraph(this, insertIndex, new XElement(w + "p"));

                        // HACK - if you try and put in a blank line, it gets ignored when DocX
                        // rebuilds the runs and the paragraphs, so we'll get around that
                        // by putting a space on the empty lines.
                        if (string.IsNullOrEmpty(line.Text))
                            line.Text = " ";

                        newParagraph.InsertText(0, line.Text, trackChanges, line.Formatting);
                        newParagraph.SetTabStops(line.TabStops);
                        InsertParagraph(insertIndex, newParagraph);
                        paragraphBeforeIndex++;
                        newParagraph.Alignment = line.Alignment;
                    }
                }
            }
        }
